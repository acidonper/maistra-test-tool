...
var (
	...
	userWorkloadEnabled   string = util.Getenv("USERWORKLOADMONITORING", "false")
	userWorkloadNamespace string = util.Getenv("USERWORKLOADNAMESPACE", "openshift-user-workload-monitoring")
	userWorkloadsecret    string = util.Getenv("USERWORKLOADMONITORINGSECRET", "thanos-querier-web-token")
	...
	testWithStages        string = util.Getenv("TESTWITHSTAGES", "false")
)
...
var (
	prometheusMeshAPIMap map[string]string = map[string]string{
		...
		"xds_write_timeouts":              "sum(rate(pilot_xds_write_timeout{app='istiod'}[1m]))",
		"pilot_total_xds_internal_errors": "sum(rate(pilot_total_xds_internal_errors{app='istiod'}[1m]))",
		"pilot_total_xds_rejects":         "sum(rate(pilot_total_xds_rejects{app='istiod'}[1m]))",
	}
...
var (
	...
	scriptFile           string = util.Getenv("SCRIPTFILE", "http-basic.js")
)
